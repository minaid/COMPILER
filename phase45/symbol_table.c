...
struct SymbolTableEntry *symbol_table_insert(void *d, enum SymbolTableType type)
{
	...
	if(type == USERFUNC || type == LIBFUNC){
		...
		// phasi 3
		tmp->typ = (type == USERFUNC)?(programfunc_s):(libraryfunc_s);
		tmp->name = (char *)((Function *)d)->name;
		tmp->scope = ((Function *)d)->scope;
		tmp->line = ((Function *)d)->line; 
	}else{
		...
		// phasi 3
		tmp->typ = var_s;
		tmp->name = (char *)((Variable *)d)->name;
		tmp->scope = ((Variable *)d)->scope;
		tmp->line = ((Variable *)d)->line;
	}
